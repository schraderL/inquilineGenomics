...
do
  ...
  tblastx -db $genomeFa -query ORexons.fa -out ORexons.bls -num_threads $cpu -evalue 1e-2 -outfmt 6
  perl $scripts/bls2gff.pl ORexons.bls > $species.OR.exons.${qspecies[$i]}.bls.gff3
  perl $EVMtools/gff3_gene_prediction_file_validator.pl $species.OR.exons.${qspecies[$i]}.bls.gff3
