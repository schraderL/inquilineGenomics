...
# usage: bash ORannotation.sh <speciesAbbreviation> <genomeFasta>
...
# GeMoMa  (1.5.2) http://www.jstacs.de/index.php/GeMoMa
...
# genometools http://genometools.org/ (aka gt v1.5.9)
...
# GFFUtils https://gffutils.readthedocs.io/en/latest/
# bedtools https://bedtools.readthedocs.io/en/latest/
# faSomeRecords http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/faSomeRecords
# pfam_scan.pl https://www.ebi.ac.uk/seqdb/confluence/display/THD/PfamScan
...
export config1=~/data/OR/config_raw
export config2=~/data/OR/config_raw2
export weights=~/data/OR/EVM.weights.txt
...
export gtools=
export transdecoder=$software/TransDecoder-3.0.1/util
...
#######################################
# All variable are set at this point. #
#######################################
...
if [[ ! -f $genomeFa  ]];
then
    echo "File $genomeFa not found" && exit 0
fi

if [[ ! -f $base/config_raw  ]];
then
    echo "File $base/config_raw not found" && exit 0
fi

if [[ ! -f $base/config_raw2  ]];
then
    echo "File $base/config_raw2 not found" && exit 0
fi

if [[ ! -f $base/EVM.weights.txt  ]];
then
    echo "File $base/EVM.weights.txt not found" && exit 0
fi


###################################################
#==================================================
# 0b. prepare folders for annotation
#==================================================
###################################################
...
if [[ ! -f $base/config_raw  ]];
then
  echo "cp $config1 $base/config_raw"
  cp $config1 $base/config_raw
fi

if [[ ! -f $base/config_raw2  ]];
then
  echo "cp $config2 $base/config_raw2"
  cp $config2 $base/config_raw2
fi


if [[ ! -f $base/EVM.weights.txt  ]];
then
  echo "cp $weights $base/EVM.weights.txt"
  cp $weights $base/EVM.weights.txt
fi
...
GFFcleaner --clean-replace-attributes --add-missing-ids --add-exon-ids --insert-missing= tmp.gff3
# Clean up the gff
$gtools/gt gff3 -sort -tidy  -checkids yes -retainids yes -fixregionboundaries tmp_clean.gff > $species.OR.gff3
# validate that the gff is ok now.
perl $EVMtools/gff3_gene_prediction_file_validator.pl $species.OR.gff3
...
mkdir stats
cat $species.OR.gff3|$gtools/gt stat --genelengthdistri -o stats/$species.genelength.stats.txt
cat $species.OR.gff3|$gtools/gt stat -genescoredistri -o stats/$species.genescore.stats.txt
cat $species.OR.gff3|$gtools/gt stat -exonlengthdistri -o stats/$species.exonlength.stats.txt
cat $species.OR.gff3|$gtools/gt stat -exonnumberdistri -o stats/$species.exonnumber.stats.txt
cat $species.OR.gff3|$gtools/gt stat -intronlengthdistri -o stats/$species.intronlength.stats.txt
cat $species.OR.gff3|$gtools/gt stat -cdslengthdistri -o stats/$species.cdslength.stats.txt
