...
https://itol.embl.de/tree/128176106115458661533028622#
cd $base
cat */final/finalSet/*.tsv|cut -f 33|egrep "dH" -v|cut -f 1 -d "-"|sort|uniq -c

#################################################################
# 8. Update inquilines to v2.1
#################################################################
# make sure everything is clean
# only hits >PargOr-269-fd scaffold301:2905-7678(+) because scaffold30 is a contaminant in Parg
for file in $(find ~/data/inqGen18/inquilines_v2.1/ -name "*contaminants.lst")
do
  short=$(echo $file|rev|cut -f 1 -d "/" |rev|perl -pe 's/(.).*?_(...).*/$1$2/g')
  grep -f  $file  ~/data/inqGen18/ORs/$short/final/finalSet/$short.OR.fa -o
done
