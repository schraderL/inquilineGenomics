...
annotationTable3<-annotationTable2[apply(annotationTable2, 1, var)> 1,]
color.palette  <- colorRampPalette(c(goodCols[1],goodCols[8]))(n=50)
heatmap.2(t(as.matrix(annotationTable3)), col = color.palette,scale = "col",trace="none")
...
pK1<-pK1+scale_x_discrete(name =paste("perm. test: log2(k) < 0; p=",ifelse(kPT$p.value==0,"1e-16",round(kPT$p.value,3)),"\n\nn=20000",sep="")) + theme(axis.title.x = element_text(size = 10))
