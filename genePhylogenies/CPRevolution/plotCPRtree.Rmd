---
title: "Inquiline Genomics: ELONGASE gene tree analysis and plot"
output:
  html_document:
    toc: true
    toc_float: true
...
############################################
# Load unrooted FastTree tree
############################################
#t1<-read.tree("/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.complete.fast.tre")
t1<-read.tree("/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.incomplete.fast.tre")
t1<-midpoint.root(t1)
tree <- as(t1, 'TreeMan')
```

## Load tsv with information for each gene
Focus on domain-containing genes ("Chitin_bind_4"), include incomplete genes (!="M*") 

```{r}
############################################
# Load subfam assignments
############################################
subfam<-read.table("/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.details.tsv",sep="\t")
# for complete
#subfam<-subset(subfam,V3=="M*" & V2=="Chitin_bind_4")
# for incomplete
subfam<-subset(subfam,V2=="Chitin_bind_4")
...
dev.print(pdf,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.heatmap.pdf",width=4,height=7)
```
...
# 1st tier: species
p1 <- gheatmap(circ, data.frame(subfamTrait), offset=0.3, width=.02,
               colnames_angle=90, colnames_offset_y = 0.5,color = NA,colnames = F)
# 2nd tier: Clades changing in parasites
p2 <- gheatmap(p1, data.frame(smallerCladesTrait), offset=.24, width=.02,
...
dev.print(pdf,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.legend.pdf",width=10,height=10)
```
...
tree_labeled <- 
  ...
  geom_text(data = coord_groups,
            ...
            vjust = 0.5, 
            size  = 4,
            nudge_x = 0.042, # Offsetting label from its default x coordinate.
            color = "black")
```

# Plot final tree
```{r}
tree_labeled + geom_tiplab2(size=1.5, aes(angle=angle),align=T,linesize=.1,linetype = "dashed", color="gray90") + geom_tiplab2(size=1.5, aes(angle=angle),align=T,linesize=0,linetype = NA, color="black")
dev.print(pdf,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.tree.pdf",width=7.5,height=7.5)
...
# save some things
save(cladesTree,file="/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.labeled.tree.Robject")
write.table(tipDF4,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.tipsDF4.tsv",quote=F,sep="\t",row.names = F)
write.table(tree_dt,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.tree_dt.tsv",quote=F,sep="\t",row.names = F)
write.table(cladeTable,"/Users/lukas/sciebo/inquilineGenomics18/geneFamilies/specificFamilies/CPRevolution/CPR.cladeTable.tsv",sep="\t",quote = F,row.names=F)
```
