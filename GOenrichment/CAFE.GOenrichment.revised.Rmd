...
## smaller in hosts
#ClustersOfInterest<-unlist(subset(all,(AINS > AECH) & (PARG > AHEY & ACHA > AHEY),ClusterID))
#ClustersOfInterest<-unlist(subset(all,(AINS > AECH),ClusterID))
#ClustersOfInterest<-unlist(subset(all,(PARG > AHEY & ACHA > AHEY),ClusterID))
#ClustersOfInterest<-unlist(subset(all,(PARG > AHEY),ClusterID))


## smaller in PARG
#ClustersOfInterest<-unlist(subset(all,ACHAp<0.1 & (ACHA > AHEY),ClusterID))
...
table_MF <- GenTable(GOdata_MF, parentChild = resultFis_MF, orderBy = "parentChild", ranksOf = "parentChild", topNodes = length(usedGO(object = GOdata_MF)))

# Biological Process
####################
resultFis_BP <- runTest(GOdata_BP, algorithm = "parentchild", statistic = "fisher")
table_BP <- GenTable(GOdata_BP, parentChild = resultFis_BP, orderBy = "parentChild", ranksOf = "parentChild", topNodes = length(usedGO(object = GOdata_BP)))


# Cellular Component
####################
resultFis_CC <- runTest(GOdata_CC, algorithm = "parentchild", statistic = "fisher")
table_CC <- GenTable(GOdata_CC, parentChild = resultFis_CC, orderBy = "parentChild", ranksOf = "parentChild", topNodes = length(usedGO(object = GOdata_CC)))
...
MF <- subset(table_MF, as.numeric(parentChild) < 0.05)
BP <- subset(table_BP, as.numeric(parentChild) < 0.05)
CC <- subset(table_CC, as.numeric(parentChild) < 0.05)
...
res<-rbind(MF,BP) %>% rbind(.,CC)
outfile<-"/Users/lukas/sciebo/inquilineGenomics18/GOenrichment/results/GOenrichment.CAFE.parasites.revision.tsv"
write.table(res,outfile,quote=F,sep="\t",row.names = F)
...
table_BP$FDR<-p.adjust(as.numeric(table_BP$classic),method="holm")
...
# All families smaller in parasite species than host species
res<-rbind(MF,BP) %>% rbind(.,CC)
outfile<-"/Users/lukas/sciebo/inquilineGenomics18/GOenrichment/results/GOenrichment.CAFE.parasites.revision.FDR.tsv"
write.table(res,outfile,quote=F,sep="\t",row.names = F)
cat("# GO enrichment of gene families smaller in parasites than in hosts. \n",file=outfile)
write.table(res, outfile,append=TRUE, quote=F,sep="\t",row.names=F)
