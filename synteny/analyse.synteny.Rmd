...
#willy<-wilcox_test(c(ksets[["ACOL"]][["AECH"]],ksets[["ACOL"]][["AHEY"]]),c(ksets[["ACOL"]][["AINS"]],ksets[["ACOL"]][["ACHA"]],ksets[["ACOL"]][["PARG"]]))


val1<-c(ksets[["ACOL"]][["AECH"]],ksets[["ACOL"]][["AHEY"]])
val2<-c(ksets[["ACOL"]][["AINS"]],ksets[["ACOL"]][["ACHA"]],ksets[["ACOL"]][["PARG"]])
f<-as.factor(c(rep("H",length(val1)),rep("P",length(val2))))
wt<-wilcox_test(c(val1,val2)~f,distribution = approximate(nresample = 1000000),alternative="greater")
pt<-perm.test(val1[!is.na(val1)],val2[!is.na(val2)],stat=median)
#text(paste("p <",round(pt$p.value,3)),y=1.005,x=2.75,font=2)
text(paste("p <",round(pvalue(wt)[1],3)),y=1.005,x=2.75,font=2)
dev.print(pdf,"/Users/lukas/sciebo/inquilineGenomics18/synteny/results/output2/synteny.pdf",width=4,height=6)
