...
for genome in $(readlink -f  $genomeBase/*/annotation/gene_annotation/*.pep.fa)
do
  species=$(echo $genome|perl -pe 's/.*\/(.).*\_(...).*/$1$2/g')
  out=$(echo $genome|rev|cut -f 1 -d "/" |rev)
  nice  python ~/software/buscoV3/scripts/run_BUSCO.py --in $species --out $out.prot --lineage $baseDB/datasets/hymenoptera_odb9 --mode prot --cpu 20
done
